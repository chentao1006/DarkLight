...
async function trackAptabaseEvent(eventName) {
  ...
  if (!permissions.data_collection?.includes('technicalAndInteraction')) return;
  ...
  try {
    await fetch(APTABASE_EVENTS_URL, {
      method: 'POST',
      headers: {
    ...
    // Analytics must never affect extension startup.
  }
}
...
chrome.runtime.onStartup.addListener(() => {
  void trackAptabaseEvent('extension_started');
  setBadgeOff();
  syncPrepaintContentScripts();
});

chrome.tabs.onActivated.addListener(({ tabId }) => refreshBadgeForTabId(tabId));
  ...
  const applySettings = (settings) => {
    ...
    chrome.scripting.getRegisteredContentScripts({}, (registeredScripts) => {
      if (chrome.runtime.lastError) {
        console.warn('[Dark Light] Unable to inspect prepaint scripts:', chrome.runtime.lastError.message);
        return;
      }
      ...
      const registerNext = () => {
        ...
        chrome.scripting.registerContentScripts(scripts, () => {
          if (chrome.runtime.lastError) {
            console.warn('[Dark Light] Unable to register prepaint scripts:', chrome.runtime.lastError.message);
          }
        });
      };
      ...
      chrome.scripting.unregisterContentScripts({ ids: registeredIds }, () => {
        if (chrome.runtime.lastError) {
          console.warn('[Dark Light] Unable to clear stale prepaint scripts:', chrome.runtime.lastError.message);
          return;
        }
        registerNext();
      });
