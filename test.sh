...
APP_NAME="Dark Light.app"
BUILT_APP_PATH="${DERIVED_DATA_DIR}/Build/Products/Release/${APP_NAME}"
BUNDLE_ID="com.ct106.darklight"
EXTENSION_BUNDLE_ID="com.ct106.darklight.Extension"
INSTALL_PATH="/Applications/${APP_NAME}"
XCODE_PROJECT="${PROJECT_ROOT}/safari/Dark Light/Dark Light.xcodeproj"
SCHEME="Dark Light"
...
echo "🛑 Stopping any running instance of ${APP_NAME}..."
pkill -x "Dark Light" || true

# Wait for process to disappear
MAX_RETRIES=10
RETRY=0
while pgrep -x "Dark Light" > /dev/null && [ $RETRY -lt $MAX_RETRIES ]; do
    echo "⏳ Waiting for Dark Light to stop... ($RETRY/$MAX_RETRIES)"
    sleep 0.5
    RETRY=$((RETRY+1))
done
...
find ~/Library/Developer/Xcode/DerivedData/Dark_Light-* -type d -name "${APP_NAME}" -exec rm -rf {} + 2>/dev/null || true
